(function () {
  const flows = [
    ...
    },
    {
      ...
      no: 4,
      title: "研究目的",
      subtitle: "把動機收束成清楚、可回答、可檢核的研究目標",
      bigIdea: "研究目的承接研究動機，必須明確說出本研究要解決什麼問題、比較哪些條件、觀察哪些結果，並能和後續研究方法與結論互相對應。",
      goals: ["能從動機提煉 2-4 個研究目的", "能分辨目的、問題與假設", "能檢查目的是否可由資料回答"],
      sequence: ["回看動機中的觀察與缺口", "列出本研究真正要回答的問題", "把目的寫成可檢核句子", "確認每個目的都有對應方法、資料與結論"],
      activities: ["動機切句：圈出可以變成目的的句子", "目的配對：把目的連到方法與圖表", "壞目的改寫：把空泛目的改成可回答目的"],
      mistakes: ["目的寫得像口號，例如了解大自然", "目的太多，超過研究能完成的範圍", "目的和後面的實驗方法或結論對不起來"],
      checklist: ["目的具體", "能用資料回答", "數量適中", "對應研究方法", "能回到結論檢核"],
      output: "2-4 點研究目的、研究問題與目的對照表",
      teacherPrompts: ["這個目的最後要用哪一張圖或哪一筆資料回答？", "這句話是在說動機、目的，還是假設？", "如果刪掉這個目的，研究還完整嗎？"],
      studentTasks: ["寫出 2-4 點研究目的", "把每一點目的配到資料來源", "檢查目的是否能在結論中回答"]
    },
    {
      id: "design",
      no: 5,
  ...
  const externalDecks = {
    ...
    purpose: {
      src: "science_fair_research_purpose_slides.html",
      pages: 16,
      label: "完整簡報"
    },
    design: {
